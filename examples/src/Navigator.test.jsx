...
// Start a navigation and let the library's own timers and state updates settle.
// Two separate act() passes: the first commits the history setState, the second
// gives any deferred work time to run before the stack is sampled.
...
describe('Navigator', () => {
    ...
    it('strips the animate.css v4 `animate__` prefix from child and option animation names', async () => {
        ...
        // so no animation-end event ever fires, `busy` latches true and the
        // navigator freezes permanently -- every later changePage/back is dropped.
        const onChangePage = vi.fn();
        let nav = null;
        const { div, unmount } = mount(
            <Navigator changeRoute={false} onChangePage={onChangePage} onRef={(ref) => { nav = ref; }}>
                ...
                <Page
                    key="hub"
                    levelPage={1}
                    transitionIn="animate__slideInRight"
                    transitionOut="animate__slideOutRight"
                >
                    hub
                </Page>
                <Page key="itemA" levelPage={2}>item A</Page>
        ...
        // Not latched: the prefixed transitionOut still completes the way back.
        await navigate(() => nav.back());
        await finishTransition('itemA');
        expect(nav.nowPage).toBe('hub');
        expect(nav.busy).toBe(false);
